<template>
    <div id = "header">
        ...
        <div id="toolbar">
                ...
                <image-btn id = "draft-box" @click.native = "draftBoxClick"></image-btn>
                <image-btn id = "deleted-box" @click.native="deleteBoxClick"></image-btn>
        </div>
    </div>
</template>
    ...
    export  default {
        ...
        methods : {
            windowBtnClick: function(method) {
                switch(method){
                    case 'minimize':
                    this.ipcSender.send(this.ipcMethod.WINDOW_OPERATE, 'minimize');
                    break;
                    case 'maximize':
                    this.ipcSender.send(this.ipcMethod.WINDOW_OPERATE, 'maximize');
                    break;
                    case 'close':
                    this.ipcSender.send(this.ipcMethod.WINDOW_OPERATE, 'close');
                    break;           
                }
            },
            inboxClick: function() {
            ...
            draftBoxClick: function() {
                this.$router.push('/draftbox');
            },
            deleteBoxClick: function() {
                this.$router.push('/deletebox');
            }
        }
    }
</script>
